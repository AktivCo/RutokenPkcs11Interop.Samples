...
namespace PKIExtensions.CreateCSR_PKCS10_GOST3410_2012_256
{
    /*************************************************************************
    * Rutoken                                                                *
    ...
    * ключевой пары ГОСТ 34.10-2012 с длиной закрытого ключа 256 бит:                               *
    *  - установление соединения с Рутокен ЭЦП в первом доступном слоте;     *
    *  - выполнение аутентификации Пользователя;                             *
    *  - генерация ключевой пары ГОСТ 34.10-2012 (256 бит) на Рутокен;       *
    ...
    *************************************************************************/
    class CreateCSR_PKCS10_GOST3410_2012_256
    {
        // Шаблон для создания открытого ключа ГОСТ Р 34.10-2012 (256 бит)
        static readonly List<IObjectAttribute> PublicKeyAttributes = new List<IObjectAttribute>
        {
            ...
            // Метка ключа
            Helpers.factories.ObjectAttributeFactory.Create(CKA.CKA_LABEL, SampleConstants.Gost256PublicKeyLabel1),
            ...
            Helpers.factories.ObjectAttributeFactory.Create((uint) CKA.CKA_GOSTR3410_PARAMS, SampleConstants.GostR3410_256_Parameters),
            // Параметры алгоритма ГОСТ Р 34.11-2012 (256 бит). Использование этих параметров явно задает
	        // требование на генерацию ГОСТ Р 34.10-2012 (256 бит)
            Helpers.factories.ObjectAttributeFactory.Create((uint) CKA.CKA_GOSTR3411_PARAMS, SampleConstants.GostR3411_256_Parameters)
        };

        // Шаблон для создания закрытого ключа ГОСТ Р 34.10-2012 (256 бит)
        static readonly List<IObjectAttribute> PrivateKeyAttributes = new List<IObjectAttribute>
        {
            ...
            // Метка ключа
            Helpers.factories.ObjectAttributeFactory.Create(CKA.CKA_LABEL, SampleConstants.Gost256PrivateKeyLabel1),
            // Идентификатор ключевой пары #1 (должен совпадать у открытого и закрытого ключей)
            Helpers.factories.ObjectAttributeFactory.Create(CKA.CKA_ID, SampleConstants.Gost256KeyPairId1),
            // Тип ключа
            Helpers.factories.ObjectAttributeFactory.Create(CKA.CKA_KEY_TYPE, (uint) CKK.CKK_GOSTR3410),
            ...
            // Параметры алгоритма ГОСТ Р 34.10-2012 (256 бит)
            Helpers.factories.ObjectAttributeFactory.Create((uint) CKA.CKA_GOSTR3410_PARAMS, SampleConstants.GostR3410_256_Parameters),
            // Параметры алгоритма ГОСТ Р 34.11-2012 (256 бит). Использование этих параметров явно задает
	        // требование на генерацию ГОСТ Р 34.10-2012 (256 бит)
            Helpers.factories.ObjectAttributeFactory.Create((uint) CKA.CKA_GOSTR3411_PARAMS, SampleConstants.GostR3411_256_Parameters)
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                using (var pkcs11 = Helpers.factories.RutokenPkcs11LibraryFactory.LoadRutokenPkcs11Library(Helpers.factories, Settings.RutokenEcpDllDefaultPath, AppType.MultiThreaded))
                {
                    ...
                    bool isGostR3411_12_256_Supported = mechanisms.Contains((CKM)Extended_CKM.CKM_GOSTR3411_12_256);
                    Errors.Check(" CKM_GOSTR3410_KEY_PAIR_GEN isn`t supported!", isGostR3410Supported);
                    Errors.Check(" CKM_GOSTR3411_12_256 isn`t supported!", isGostR3411_12_256_Supported);
